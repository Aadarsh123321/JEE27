...
@media (max-width: 768px) {
    ...
    .exercises-container { padding: 1rem; width: 100%; }
    
    .exercise-card { flex-direction: column; align-items: flex-start; gap: 12px; }
    .progress-glow { float: none; display: inline-block; margin-top: 10px; font-size: 0.8rem; }
    h2 { display: flex; flex-direction: column; align-items: flex-start; }
    
    .practice-container { flex-direction: column; overflow-y: auto; padding: 10px; }
    .left-panel, .right-panel { width: 100%; min-width: 100%; flex: none; }
    .question-header { flex-wrap: wrap; gap: 10px; }
    .action-bar { flex-wrap: wrap; gap: 10px; justify-content: center; }
    
    .rankings-header { font-size: 1.5rem; text-align: center; flex-direction: column; }
    .lb-row { flex-direction: column; align-items: flex-start; gap: 10px; }
...
// --- 3. FIREBASE AUTH & LIVE GLOBAL LEADERBOARD DB ---
...
// STRICT FIX: This function completely controls the Leaderboard Live without drifting or resetting.
function initLiveLeaderboard() {
    ...
    // onSnapshot creates a live web-socket connection to Firestore. Any solved question updates this instantly!
    db.collection("users").orderBy("questionsSolved", "desc").limit(100)
...
auth.onAuthStateChanged(user => {
    if(user){
        ...
        // STRICT FIX: Never blindly overwrite with 0! Only set to 0 if they've completely never existed.
        ...
        userDocRef.get().then(docSnap => {
            if (!docSnap.exists) {
                userDocRef.set({
                    ...
                    photoURL: photoURL,
                    email: user.email,
                    questionsSolved: 0 // Initialize to 0 only once so they appear in Leaderboard immediately!
                });
            } else {
                userDocRef.set({
                    name: user.displayName || 'Anonymous Aspirant',
            ...
            if(key && key.startsWith(prefix)) {
                try {
                    ...
                    if (Array.isArray(data)) {
                        totalQ += data.length;
                        solvedQ += data.filter(q => q.st === 'correct').length;
                    }
                } catch(e) {}
            }
        }
...
// STRICT FIX: Mandatory Login check before opening practice!
...
function chkAns(){
    ...
    if(q.so===q.c){
        ...
        // STRICT FIX: Increment score directly and exclusively on Firestore database!
        const user = firebase.auth().currentUser;
        if (user) {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    initSpaceAnimation();
    animateSpace();
    initLiveLeaderboard(); // Initialize the websocket immediately on load
});
